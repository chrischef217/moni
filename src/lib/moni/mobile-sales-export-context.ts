...
function normalizeItemKey(item: MobileSalesExportExtractedItem) {
  return [item.name, item.specification || '', item.quantity ?? '', item.unit || '', item.cartons ?? '', item.unit_price ?? '']
    .map((value) => text(value, 300).normalize('NFKC').toLowerCase().replace(/\s+/g, ''))
    .join('|')
}
...
function normalizeOutput(raw: Record<string, unknown>): MobileSalesExportContext {
  ...
  const items = normalizedItems.filter((row) => {
