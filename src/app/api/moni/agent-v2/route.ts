...
async function loadAttachments(supabase: SupabaseClient, threadId: string, rawIds: unknown) {
  ...
  for (const row of rows) {
    ...
    if (!extractedText && (mimeType === 'text/plain' || mimeType === 'text/csv' || mimeType === 'application/json')) {
      extractedText = buffer.toString('utf8').slice(0, 120000)
    } else if (!extractedText && (mimeType === 'application/vnd.ms-excel' || mimeType === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await runMoniAgent({
      apiKey: openAIKey,
      model,
      history,
    ...
    if (isPmoRequest) {
      await reportMoniPmoEvent({
        ...
        summary: message,
        evidence: { page, tools_used: result.toolsUsed, agent_run_id: result.agentRunId },
      })
    }
