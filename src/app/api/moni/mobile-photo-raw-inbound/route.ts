...
function parseJsonObject(value: unknown): Record<string, any> | null {
  ...
  for (const candidate of candidates) {
    try { const parsed = JSON.parse(candidate); if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) return parsed } catch { /* next */ }
  }
  return null
}
...
function isExpiryFollowup(value: string) {
  return /(유통기한|소비기한|사용기한|기한|EXP|BEST\s*BEFORE|USE\s*BY|LOT)/i.test(value)
    && /(반영|입력|등록|추가|적용|저장|확인|읽어|봐|체크|해줘|해주세요|해 줘)/.test(value)
}

async function loadRecentContext(db: ReturnType<typeof createMoniServiceRoleClient>, threadId: string) {
  const result = await db.from('moni_ai_messages').select('role,content,created_at').eq('business_id', BUSINESS_ID).eq('thread_id', threadId).in('role', ['user', 'assistant']).order('created_at', { ascending: false }).limit(8)
...
async function loadMaterials(db: ReturnType<typeof createMoniServiceRoleClient>) {
  const result = await db.from('raw_materials').select('id,item_code,item_name,supplier,packing_weight_g,unit_price_per_kg,box_quantity,spec,storage_type,shelf_life_days,is_stock_managed,is_active').eq('business_id', BUSINESS_ID).eq('is_active', true).order('item_name').limit(1000)
...
async function loadAttachments(db: ReturnType<typeof createMoniServiceRoleClient>, threadId: string, ids: string[]) {
  const result = await db.from('moni_ai_attachments').select('id,file_name,mime_type,size_bytes,storage_bucket,storage_path,message_id,metadata,created_at').eq('business_id', BUSINESS_ID).eq('thread_id', threadId).eq('upload_status', 'READY').in('id', ids)
  if (result.error) throw new Error(result.error.message)
...
async function resolvePhotoWorkflowId(db: ReturnType<typeof createMoniServiceRoleClient>, threadId: string, message: string) {
  if (!isExpiryFollowup(message)) return randomUUID()
  const result = await db.from('moni_ai_attachments').select('id,metadata,created_at').eq('business_id', BUSINESS_ID).eq('thread_id', threadId).eq('upload_status', 'READY').gte('created_at', new Date(Date.now() - PHOTO_WORKFLOW_LOOKBACK_MS).toISOString()).order('created_at', { ascending: false }).limit(12)
  if (result.error) throw new Error(result.error.message)
  for (const row of result.data ?? []) {
    const metadata = row?.metadata && typeof row.metadata === 'object' ? row.metadata : {}
    const workflowId = text(metadata.raw_material_inbound_workflow_id, 80)
...
function exactMaterial(materials: any[], analysis: Record<string, any>) {
  ...
  if (id) {
    const byId = materials.find((row: any) => (String(row.id) === id || String(row.item_code || '') === id) && row.is_stock_managed === true)
    if (byId) return byId
  }
  const name = normalize(analysis.matched_material_name)
  if (!name) return null
  const exact = materials.filter((row: any) => row.is_stock_managed === true && normalize(row.item_name) === name)
...
function visibleAnswer(analysis: Record<string, any>, material: any, attachmentCount: number) {
  ...
  if (material && photoPackWeightG && masterPacking && Math.round(photoPackWeightG) !== Math.round(masterPacking)) warnings.unshift(`사진 표기 포장중량 ${formatKg(photoPackWeightG)}과 원재료 마스터 포장기준 ${formatKg(masterPacking)}이 다릅니다. 저장 전에 실제 입고 포장을 확인해야 합니다.`)
  if (packCount && countConfidence < 0.75) warnings.unshift('사진에서 센 포장 개수의 확신도가 낮습니다. 저장 전에 실제 개수를 확인해 주세요.')
  if (!material) warnings.unshift('사진만으로 두배 원재료 마스터의 정확한 재고관리 항목을 하나로 확정하지 못했습니다. 카드에서 원재료를 직접 선택해 주세요.')
  return [
...
export async function POST(request: NextRequest) {
  try {
    ...
    const thread = await db.from('moni_ai_threads').select('*').eq('id', threadId).eq('business_id', BUSINESS_ID).eq('user_login_id', session.loginId).eq('status', 'ACTIVE').maybeSingle()
    ...
    if (!isRawMaterialPhotoInboundRequest(message, recentContext)) return NextResponse.json({ ok: false, code: 'NOT_RAW_MATERIAL_PHOTO_INBOUND', error: '원재료 입고 사진 요청이 아닙니다.' }, { status: 422 })
    ...
    const [materials, attachments, workflowId] = await Promise.all([loadMaterials(db), loadAttachments(db, threadId, attachmentIds), resolvePhotoWorkflowId(db, threadId, message)])
    ...
    const linked = await db.from('moni_ai_attachments').update({ message_id: userMessage.data.id, updated_at: new Date().toISOString() }).eq('business_id', BUSINESS_ID).eq('thread_id', threadId).in('id', attachmentIds)
    if (linked.error) throw new Error(linked.error.message)

    const masterForModel = materials.map((row: any) => ({ id: row.id, code: row.item_code, name: row.item_name, supplier: row.supplier, packing_weight_g: row.packing_weight_g, box_quantity: row.box_quantity, spec: row.spec, shelf_life_days: row.shelf_life_days, stock_managed: row.is_stock_managed === true }))
    const imageContent = await toImageContent(db, attachments)
    const instructions = `당신은 MONI의 원재료 입고 사진 판독기입니다. 사진에서 실제로 보이는 증거와 제공된 두배 원재료 마스터만 사용합니다. 보이지 않거나 흐린 값은 절대 추측하지 않습니다.\n\n목표:\n1) 포장/라벨의 제품명·브랜드·규격 글자를 읽고 제공된 원재료 마스터에서 정확히 대응되는 재고관리(stock_managed=true) 항목이 하나일 때만 matched_material_id/name을 선택합니다. 유사하지만 확정할 수 없거나 재고관리 미설정이면 null입니다.\n2) 사진에서 서로 구분 가능한 실제 포장 개수만 package_count로 셉니다. 가려짐/중첩 때문에 확실하지 않으면 null입니다.\n3) 포장 하나에 적힌 중량만 package_weight_g로 기록합니다. 총중량이 별도 표기된 경우에만 total_weight_g를 기록합니다.\n4) 소비기한/유통기한/EXP/BEST BEFORE/USE BY가 사진에 명확히 보일 때만 expiry_kind와 expiry_date(YYYY-MM-DD)를 기록합니다. 제조일자를 기한으로 바꾸지 않습니다.\n5) 여러 사진은 같은 입고 건의 보완 증거일 수 있습니다. 추가 사진이 기한 라벨만 보여주고 최근 대화에서 원재료가 이미 확정되어 있으면 recent_context의 확정된 마스터를 이어서 사용할 수 있지만, 사진과 대화가 충돌하면 null로 둡니다.\n6) unit price, supplier, packing master data는 사진으로 만들지 않습니다. 이 값들은 서버가 선택된 마스터에서 다시 조회합니다.\n\n반드시 JSON 객체 하나만 출력하세요. 설명문/마크다운 금지.\n{\n  "matched_material_id": string|null,\n  "matched_material_name": string|null,\n  "match_confidence": number,\n  "match_source": "photo"|"recent_context"|"none",\n  "visible_product_text": string,\n  "package_count": integer|null,\n  "package_count_confidence": number,\n  "package_weight_g": number|null,\n  "total_weight_g": number|null,\n  "expiry_kind": "소비기한"|"유통기한"|"EXP"|null,\n  "expiry_date": "YYYY-MM-DD"|null,\n  "expiry_raw_text": string|null,\n  "lot_text": string|null,\n  "warnings": string[]\n}\n\n[최근 대화]\n${text(recentContext, 8000) || '없음'}\n\n[두배 원재료 마스터 후보]\n${JSON.stringify(masterForModel).slice(0, 24000)}`

    const analyzer = new Agent({ name: 'MONI Raw Material Inbound Photo Analyzer', model: modelName(), modelSettings: { parallelToolCalls: false, reasoning: { effort: 'minimal' }, text: { verbosity: 'low' }, maxTokens: 900 }, instructions })
    const startedAt = Date.now()
    const result = await run(analyzer, [{ role: 'user', content: [{ type: 'input_text', text: message }, ...imageContent] }] as any, { maxTurns: 1 })
    const parsed = parseJsonObject(typeof result.finalOutput === 'string' ? result.finalOutput : JSON.stringify(result.finalOutput ?? {})) || { matched_material_id: null, matched_material_name: null, match_confidence: 0, visible_product_text: '', package_count: null, package_count_confidence: 0, package_weight_g: null, total_weight_g: null, expiry_kind: null, expiry_date: null, warnings: ['사진 분석 결과를 구조화하지 못했습니다. 카드에서 값을 직접 확인해 주세요.'] }
    const material = exactMaterial(materials, parsed)
    if (!material) { parsed.matched_material_id = null; parsed.matched_material_name = null; parsed.match_confidence = 0 }
    else { parsed.matched_material_id = material.id; parsed.matched_material_name = material.item_name }
    const answer = visibleAnswer(parsed, material, attachmentIds.length)
    const now = new Date().toISOString()

    const assistantMessage = await db.from('moni_ai_messages').insert({ business_id: BUSINESS_ID, thread_id: threadId, role: 'assistant', content: answer, page_context: page, provider: 'openai', model: modelName() }).select('id').single()
    if (assistantMessage.error) throw new Error(assistantMessage.error.message)

    for (const attachment of attachments) {
      const existingMetadata = attachment?.metadata && typeof attachment.metadata === 'object' && !Array.isArray(attachment.metadata) ? attachment.metadata : {}
      const updated = await db.from('moni_ai_attachments').update({
        metadata: { ...existingMetadata, analysis_kind: 'raw_material_inbound_photo', raw_material_inbound_workflow_id: workflowId, raw_material_inbound_analysis: parsed, matched_material_id: material?.id || null, analyzed_at: now },
        updated_at: now,
      }).eq('business_id', BUSINESS_ID).eq('thread_id', threadId).eq('id', attachment.id)
      if (updated.error) throw new Error(updated.error.message)
    }

    const agentRun = await db.from('moni_ai_agent_runs').insert({ business_id: BUSINESS_ID, thread_id: threadId, message_id: userMessage.data.id, provider: 'openai', model: modelName(), status: 'COMPLETED', validation_status: material ? 'VERIFIED' : 'NEEDS_REVIEW', prompt_version: 'MONI_RAW_MATERIAL_PHOTO_INBOUND_V1', step_count: 1, tool_call_count: 0, request_count: 1, input_tokens: 0, output_tokens: 0, total_tokens: 0, latency_ms: Date.now() - startedAt, finished_at: now, usage: { requests: 1, inputTokens: 0, outputTokens: 0, totalTokens: 0 }, metadata: { raw_material_photo_inbound: true, workflow_id: workflowId, attachment_ids: attachmentIds, matched_material_id: material?.id || null, analysis: parsed, separate_turn_write_approval: true } }).select('id').single()
    if (agentRun.error) throw new Error(agentRun.error.message)
    await db.from('moni_ai_threads').update({ title: thread.data.title || message.replace(/\s+/g, ' ').slice(0, 80), current_page: page, updated_at: now, last_message_at: now }).eq('id', threadId).eq('business_id', BUSINESS_ID)

    return NextResponse.json({ ok: true, text: answer, provider: 'openai', model: modelName(), thread_id: threadId, assistant_message_id: assistantMessage.data.id, attachment_count: attachmentIds.length, image_context_count: attachmentIds.length, agent_runtime: 'MONI_RAW_MATERIAL_PHOTO_INBOUND_V1', agent_run_id: agentRun.data?.id || null, photo_workflow_id: workflowId, structured_action_card: true, domain: 'raw_material_inbound', operation: 'CREATE' }, { headers: { 'Cache-Control': 'no-store' } })
  } catch (error) {
