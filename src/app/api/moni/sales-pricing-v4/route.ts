...
async function loadAll(client: ReturnType<typeof createMoniServiceRoleClient>) {
  const [productsResult, variantsResult, packagingResult, clientsResult, peopleResult, clientPeopleResult, termsResult, agentsResult] = await Promise.all([
    ...
    client.from('packaging_materials').select('id,material_name,material_code,spec,material_type,ingredient_type,is_active').eq('is_active', true).order('material_name'),
  ...
  const failed = [productsResult, variantsResult, packagingResult, clientsResult, peopleResult, clientPeopleResult, termsResult, agentsResult]
  ...
  const terms = (termsResult.data ?? []).map((row) => ({
    ...row,
    agent_rates: agentsByTerm.get(text(row.id)) ?? [],
  }))
  ...
  const clients = (clientsResult.data ?? []).map((row) => ({
    ...row,
    assigned_person_ids: peopleByClient.get(text(row.id)) ?? [],
  }))

  return {
    products: productsResult.data ?? [],
    variants: variantsResult.data ?? [],
    packaging_materials: packagingResult.data ?? [],
    clients,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'save_variant') {
      ...
      let result
      if (id) {
        result = await client.from('sales_product_variants').update(payload).eq('id', id).eq('business_id', BUSINESS_ID).select('*').single()
      } else {
      ...
      if (isDefault) {
        const reset = await client.from('sales_product_variants')
    ...
    return NextResponse.json({
      ok: false,
      error: error instanceof Error ? error.message : '판매규격 저장 중 오류가 발생했습니다.',
    }, { status: 500 })
  }
}
