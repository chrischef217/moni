...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await executeProductionPlanChange(input, identity)
    return NextResponse.json({
      ok: true,
      integration: 'CHATGPT_CUSTOM_GPT_ACTIONS',
      intelligence_runtime: 'CHATGPT_PRODUCT',
      moni_server_model_inference: false,
      tool: 'execute_production_plan_change',
      result,
    }, { status: 200, headers: NO_STORE })
  } catch (error) {
    const message = error instanceof Error ? error.message : '승인된 생산계획 실행에 실패했습니다.'
    return NextResponse.json({ ok: false, tool: 'execute_production_plan_change', error: message }, { status: 400, headers: NO_STORE })
  }
}
