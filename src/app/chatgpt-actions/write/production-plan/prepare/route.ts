...
import { prepareProductionPlanChange } from '@/lib/moni/chatgpt-write-actions'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await prepareProductionPlanChange(input, identity)
    return NextResponse.json({
      ok: true,
      integration: 'CHATGPT_CUSTOM_GPT_ACTIONS',
      intelligence_runtime: 'CHATGPT_PRODUCT',
      moni_server_model_inference: false,
      tool: 'prepare_production_plan_change',
      result,
    }, { status: 200, headers: NO_STORE })
  } catch (error) {
    const message = error instanceof Error ? error.message : '생산계획 실행 미리보기 생성에 실패했습니다.'
    return NextResponse.json({ ok: false, tool: 'prepare_production_plan_change', error: message }, { status: 400, headers: NO_STORE })
  }
}
