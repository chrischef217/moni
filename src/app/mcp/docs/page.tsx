...
const readiness = [
  ...
  { label: '최대 30분 자동 만료 수용검사 창', state: '완료' },
  { label: '수용검사 자격증명 창 밖 재사용 차단', state: '완료' },
...
export default async function MoniMcpDocsPage() {
  const activation = await getMoniMcpActivationState()
  const activationLabel = activation.mode === 'PERMANENT_ENV'
    ? '영구 운영 활성'
    : activation.mode === 'ACCEPTANCE_WINDOW'
      ? `수용검사 창 활성 — ${formatDate(activation.enabledUntil)} 자동 종료`
      : '비활성 — 실제 ChatGPT 수용검사 전 보호 상태'
      ...
      <article className="mx-auto max-w-3xl rounded-3xl border border-slate-200 bg-white p-8 shadow-xl md:p-12">
        ...
        <section className="mt-8 grid gap-4 rounded-2xl bg-emerald-50 p-6">
          ...
          <ul className="list-disc space-y-2 pl-5 text-sm leading-6 text-emerald-900">
            ...
            <li>수용검사 연결은 최대 30분 후 자동 차단되며 테스트 자격증명은 이후 재사용할 수 없습니다.</li>
          </ul>
        </section>
        ...
        <section className="mt-8">
          ...
          <dl className="mt-4 grid gap-3 text-sm">
            ...
            <div className="rounded-xl border border-slate-200 p-4">
              ...
              <dd className={`mt-1 font-black ${activation.enabled ? 'text-emerald-700' : 'text-amber-700'}`}>
                {activationLabel}
              </dd>
            </div>
          </dl>
        ...
        <section className="mt-8 rounded-2xl border border-amber-200 bg-amber-50 p-6">
          ...
          <p className="mt-2 text-sm leading-6 text-amber-900">
            서버 기반·보안·DB 보호와 자동 만료 수용검사 창까지 준비됐습니다. 실제 ChatGPT 지원 플랜에서 도구 스캔과 역할별 실데이터 교차검산을 완료하기 전까지 영구 운영 플래그는 켜지 않습니다.
