...
const CURRENCIES = ['KRW', 'USD', 'THB', 'EUR'] as const
...
export default function ExportProductSettingsPage() {
  ...
  function openEdit(setting: ExportSetting) {
    setForm({
      ...
      hs_code: setting.hs_code || DEFAULT_HS_CODE,
      default_unit_price: String(setting.default_unit_price ?? ''),
      currency: setting.currency || 'KRW',
  ...
  async function save() {
    ...
    if (!form.hs_code.trim()) return setError('HS CODE를 입력해 주세요.')
      ...
      <header className="rounded-[26px] border border-[#cfe1eb] bg-white/95 p-6 shadow-[0_14px_36px_rgba(43,84,109,0.08)] lg:p-8">
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-[#6b8392]">수출할 완제품만 등록하고 제품별 HS CODE, 실제 1카톤(CTN) 입수량, 단가, 중량과 CBM을 설정합니다.</p>
          </div>
        ...
        <div className="mt-5 rounded-2xl border border-[#d5e5ed] bg-[#f7fbfd] px-5 py-4 text-sm leading-6 text-[#637d8c]">
          <b className="text-[#315469]">기본값:</b> HS CODE <b>{DEFAULT_HS_CODE}</b> · 통화 <b>KRW</b>. 제품별로 필요한 경우 수정할 수 있습니다.
        </div>
      </header>
      ...
      <section className="overflow-hidden rounded-[26px] border border-[#cfe1eb] bg-white/95 shadow-[0_12px_34px_rgba(43,84,109,0.07)]">
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-sm">
            <colgroup>{Array.from({ length: 10 }).map((_, index) => <col key={index} />)}</colgroup>
            <thead><tr className="bg-[#f1f7fb] text-xs font-bold text-[#667f8f]"><th className="px-4 py-4">완제품</th><th className="px-3 py-4">영문이름</th><th className="px-3 py-4">HS CODE</th><th className="px-3 py-4">입수량</th><th className="px-3 py-4">Unit Price / CTN</th><th className="px-3 py-4">Net Weight / CTN</th><th className="px-3 py-4">Gross Weight / CTN</th><th className="px-3 py-4">CBM / CTN</th><th className="px-3 py-4">상태</th><th className="px-4 py-4">관리</th></tr></thead>
            <tbody>
              {settings.map((setting) => <tr key={setting.id} className="border-t border-[#e7eff4] bg-white hover:bg-[#f9fcfd]">
                <td className="px-4 py-4"><div className="font-black text-[#17384d]">{setting.products?.product_name || '-'}</div><div className="mt-1 text-xs text-[#8296a3]">품목제조번호 {setting.products?.report_number || '미등록'} · {setting.products?.product_spec || '규격 미등록'}</div></td>
                <td className="px-3 py-4 font-bold text-[#31546a]">{setting.english_name}</td>
                <td className="px-3 py-4 font-black text-[#485f70]">{setting.hs_code || DEFAULT_HS_CODE}</td>
                <td className="px-3 py-4"><span className="inline-flex items-center justify-center rounded-lg bg-[#eef7f3] px-2 py-1.5 font-black text-[#227a59]">{setting.units_per_carton ? `${setting.units_per_carton} EA / CTN` : '-'}</span></td>
                <td className="px-3 py-4 font-black text-[#176f99]">{priceText(setting.default_unit_price, setting.currency)}</td>
                <td className="px-3 py-4 font-semibold">{numberText(setting.net_weight_kg)} kg</td>
                <td className="px-3 py-4 font-semibold">{numberText(setting.gross_weight_kg)} kg</td>
                <td className="px-3 py-4 font-semibold">{numberText(setting.cbm, 6)}</td>
                <td className="px-3 py-4"><span className={`inline-flex items-center justify-center rounded-lg px-2.5 py-1.5 text-xs font-black ${setting.is_active ? 'bg-[#eaf8f2] text-[#16825d]' : 'bg-[#f1f4f6] text-[#7c8c96]'}`}>{setting.is_active ? '사용' : '미사용'}</span></td>
                <td className="px-4 py-4"><div className="flex flex-nowrap justify-center gap-1.5"><button type="button" onClick={() => openEdit(setting)} className="rounded-lg border border-[#bfd5e1] bg-white px-2.5 py-2 text-xs font-black text-[#315d75]">수정</button><button type="button" onClick={() => void remove(setting)} className="rounded-lg border border-[#efb9bf] bg-[#fffafa] px-2.5 py-2 text-xs font-black text-[#b44f58]">삭제</button></div></td>
              </tr>)}
              {!settings.length && <tr><td colSpan={10} className="px-6 py-16 text-center"><div className="text-lg font-black text-[#31546a]">등록된 수출품목이 없습니다.</div><div className="mt-2 text-sm text-[#8296a3]">‘수출 품목 등록’을 눌러 필요한 완제품만 등록하세요.</div></td></tr>}
            </tbody>
          </table>
        </div>
      </section>
    ...
    {modalOpen && <div className="fixed inset-0 z-[1300] flex items-center justify-center bg-[rgba(12,31,44,0.34)] p-4 backdrop-blur-[3px]" onMouseDown={(event) => { if (event.currentTarget === event.target) closeModal() }}>
      <div className="flex max-h-[90vh] w-full max-w-[980px] flex-col overflow-hidden rounded-[26px] border border-[#cfe1eb] bg-white shadow-[0_28px_80px_rgba(22,52,72,0.24)]">
        <div className="flex items-start justify-between gap-4 border-b border-[#dce9f0] px-6 py-5">
          <div><p className="text-xs font-black uppercase tracking-[0.14em] text-[#2b9b76]">EXPORT ITEM</p><h2 className="mt-1 text-2xl font-black">{form.id ? '수출품목 수정' : '수출 품목 등록'}</h2><p className="mt-1 text-sm text-[#718896]">기존 완제품을 연결하고 수출 통관·카톤 기준 정보를 설정합니다.</p></div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
          ...
          <div className="rounded-2xl border border-[#d5e5ed] bg-[#f7fbfd] p-4">
            ...
            <p className="mt-1 text-xs text-[#7c919e]">제품명, 품목제조번호 또는 제품코드로 검색합니다.</p>
            <input disabled={Boolean(form.id)} value={productSearch} onChange={(event) => { setProductSearch(event.target.value); setForm((current) => ({ ...current, product_id: '' })) }} placeholder="예: 마늘소스 / 품목제조번호" className="mt-3 h-12 w-full rounded-xl border border-[#cfe0e9] bg-white px-4 text-sm font-semibold outline-none focus:border-[#7fb9d1] disabled:bg-[#eef3f6]" />
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            ...
            <label><span className="mb-1.5 block text-sm font-bold text-[#5f7888]">상태</span><div className="grid h-12 grid-cols-2 overflow-hidden rounded-xl border border-[#cfe0e9]"><button type="button" onClick={() => setForm((current) => ({ ...current, is_active: true }))} className={`font-black ${form.is_active ? 'bg-[#176f53] text-white' : 'bg-white text-[#5f7888]'}`}>사용</button><button type="button" onClick={() => setForm((current) => ({ ...current, is_active: false }))} className={`border-l border-[#d7e4eb] font-black ${!form.is_active ? 'bg-[#6b7f8b] text-white' : 'bg-white text-[#5f7888]'}`}>미사용</button></div></label>
          </div>
        </div>

        <div className="flex items-center justify-between gap-4 border-t border-[#dce9f0] bg-[#f8fbfd] px-6 py-4">
          <p className="text-xs leading-5 text-[#728a98]">HS CODE와 통화·단가는 수출서류 저장 시 해당 문서에 스냅샷으로 보존됩니다.</p>
