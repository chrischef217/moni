...
export default function MoniMobileChat() {
  ...
  function currentVoiceDraft() {
    return [voiceSeedRef.current, voiceFinalRef.current, voiceInterimRef.current]
      .map((part) => part.trim())
      .filter(Boolean)
      .join(' ')
      .trim()
  ...
  function finishVoiceInput() {
    // Confirmation only converts speech into a draft. It never sends to MONI.
    commitVoiceDraft()
  ...
  async function startVoiceInput() {
    ...
    try {
      ...
      recognition.onresult = (event) => {
        ...
        // Keep the latest speech mirrored into the actual composer draft continuously.
        // This prevents Android Chrome from losing the last interim result when the user taps 확인.
        const draft = currentVoiceDraft()
        if (draft) setInput(draft)
      }
    ...
    <>
          ...
          <div className={`overflow-hidden rounded-[28px] border bg-white shadow-[0_8px_32px_rgba(23,59,82,0.12)] transition ${listening ? 'border-violet-300 ring-4 ring-violet-100/70' : 'border-[#d2dfdc]'}`}>
            {listening ? (
              <div className="flex min-h-[72px] items-center gap-3 px-3 py-2">
                <button type="button" onClick={finishVoiceInput} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#18181b] text-white" aria-label="음성 입력 확인">
                ...
                <button type="button" onClick={finishVoiceInput} className="h-10 shrink-0 rounded-full bg-violet-100 px-3 text-xs font-black text-violet-700">확인</button>
              </div>
            ) : (
      ...
      <style jsx global>{`
        ...
        .moni-mobile-character {
          position: relative; width: 54px; height: 54px; flex: 0 0 auto; border: 1px solid rgb(255 255 255 / 0.25); border-radius: 19px; background: #0c2337;
          box-shadow: 0 10px 26px rgb(2 6 23 / 0.20); animation: moniMobileFloat 2.7s ease-in-out infinite; transform-origin: center; transition: box-shadow 180ms ease, border-color 180ms ease;
        }
        ...
        .moni-mobile-antenna-stem { position: absolute; top: -7px; left: 50%; width: 3px; height: 9px; transform: translateX(-50%); border-radius: 999px; background: rgb(110 231 183 / 0.80); }
        .moni-mobile-antenna-dot { position: absolute; top: -12px; left: 50%; width: 8px; height: 8px; transform: translateX(-50%); border: 1px solid rgb(209 250 229 / 0.42); border-radius: 999px; background: rgb(52 211 153); box-shadow: 0 0 12px rgb(110 231 183 / 0.65); }
        .moni-mobile-face-glow { position: absolute; inset: 3px; border-radius: 16px; background: linear-gradient(135deg, rgb(110 231 183 / 0.20), rgb(103 232 249 / 0.10) 52%, rgb(59 130 246 / 0.20)); }
        ...
        .moni-mobile-mouth { position: absolute; left: 50%; bottom: 12px; width: 14px; height: 7px; transform: translateX(-50%); border-bottom: 2px solid rgb(209 250 229 / 0.92); border-radius: 0 0 12px 12px; animation: moniMobileSmile 1.55s ease-in-out infinite; transform-origin: center bottom; }
        ...
        @keyframes moniMobileBlink { 0%, 34%, 42%, 100% { transform: scaleY(1); } 38% { transform: scaleY(0.08); } }
        @keyframes moniMobileSmile { 0%, 100% { transform: translateX(-50%) scaleX(0.92) scaleY(0.88); } 50% { transform: translateX(-50%) scaleX(1.08) scaleY(1.08); } }
        ...
        @keyframes moniLivePulse { 0% { box-shadow: 0 0 0 0 rgb(239 68 68 / 0.48); opacity: 1; } 70% { box-shadow: 0 0 0 7px rgb(239 68 68 / 0); opacity: 0.7; } 100% { box-shadow: 0 0 0 0 rgb(239 68 68 / 0); opacity: 1; } }
        ...
        .moni-markdown ul, .moni-markdown ol { margin: 6px 0 11px 20px; padding: 0; }
        ...
        .moni-markdown th:last-child, .moni-markdown td:last-child { border-right: 0; }
        ...
        @media (prefers-reduced-motion: reduce) {
          .moni-thinking-dot, .moni-mobile-character, .moni-mobile-eye, .moni-mobile-mouth, .moni-live-dot { animation: none !important; }
        }
      `}</style>
    </>
  )
