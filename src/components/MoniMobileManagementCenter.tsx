...
type HistoryRow = { id: string; title: string; subtitle?: string; meta?: string; badges?: string[]; links?: Link[] }
...
export default function MoniMobileManagementCenter() {
  ...
  const query = useCallback(async (nextFilters?: Record<string,string>) => {
    ...
    try {
      ...
      const response = await fetch('/api/moni/mobile-management-center', {
        ...
        body:JSON.stringify({ command:'query', thread_id:threadId(), source_user_message_id:currentCard.source_user_message_id, domain:currentCard.domain, filters:values }),
      })
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (sourceRef.current !== key) {
        ...
        const initial = Object.fromEntries((next.filters || []).map((item) => [item.key, item.value || ''])) as Record<string,string>
        initial.page = '1'
        setFilters(initial); setResult(null); setError(''); setPendingFinance(null); setConfirmation(null)
        if (autoQueriedRef.current !== key) {
          autoQueriedRef.current = key
          window.setTimeout(() => {
            void fetch('/api/moni/mobile-management-center', {
              method:'POST', headers:{'Content-Type':'application/json'},
              body:JSON.stringify({ command:'query', thread_id:id, source_user_message_id:next.source_user_message_id, domain:next.domain, filters:initial }),
            }).then(async (r) => {
              const p = await r.json(); if (!r.ok || !p.ok) throw new Error(p.error || '관리 데이터를 조회하지 못했습니다.'); setResult(p.result || null)
            }).catch((reason) => setError(reason instanceof Error ? reason.message : '관리 데이터를 조회하지 못했습니다.'))
          }, 40)
        }
      }
    } catch { /* management center never blocks the chat */ }
  }, [])
  ...
  useEffect(() => {
    ...
    node.dataset.moniManagementCenterHost = 'true'
    node.className = 'moni-management-center-host'
    scroller.appendChild(node); setHost(node)
    const hide = () => { suppressedSourceRef.current = activeSourceRef.current; setCard(null); setResult(null); setError(''); setPendingFinance(null); setConfirmation(null) }
    window.addEventListener('moni:user-turn-start', hide)
    const timer = window.setInterval(() => void refresh(), 950)
    void refresh()
  ...
  useEffect(() => {
    const root = document.querySelector<HTMLElement>('[data-moni-mobile-chat]')
    if (!root) return
    if (card) root.dataset.moniManagementActive = 'true'
    else delete root.dataset.moniManagementActive
  ...
  function page(delta: number) {
    const next = String(Math.max(1, Number(filters.page || 1) + delta))
    const updated = { ...filters, page:next }
    setFilters(updated); void query(updated)
  }

  async function prepareFinance() {
    if (!card || !pendingFinance || busy) return
    setBusy(true); setError('')
    try {
      const data = pendingFinance.action === 'set_settlement_due_date' ? { due_date:pendingFinance.value }
        : pendingFinance.action === 'mark_settlement_paid' ? { paid_date:pendingFinance.value }
        : { reason:pendingFinance.value }
      ...
      const payload = await response.json(); if (!response.ok || !payload.ok) throw new Error(payload.error || '실행 내용을 확인하지 못했습니다.')
      setConfirmation(payload.confirmation)
    } catch(reason) { setError(reason instanceof Error ? reason.message : '실행 내용을 확인하지 못했습니다.') }
    finally { setBusy(false) }
  }

  async function executeFinance() {
    if (!card || !confirmation?.id || busy) return
    setBusy(true); setError('')
    try {
      const response = await fetch('/api/moni/mobile-management-center', { method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({command:'execute_finance',thread_id:threadId(),confirmation_id:confirmation.id}) })
      const payload = await response.json(); if (!response.ok || !payload.ok) throw new Error(payload.error || '실행하지 못했습니다.')
      setPendingFinance(null); setConfirmation(null)
      await query()
  ...
  function startFinance(settlement: TaxSettlement, action: PendingFinance['action']) {
    ...
    setPendingFinance({ settlement, action, value }); setConfirmation(null); setError('')
  }
  ...
  return createPortal(<>
    <section className="moni-mgmt-card">
      ...
      <button type="button" disabled={busy} className="moni-mgmt-primary" onClick={() => void query({ ...filters, page:'1' })}>{busy ? '조회 중…' : '조건으로 다시 조회'}</button>
      ...
      {result && isTax(result) ? <div className="moni-tax-wrap">
        <div className="moni-tax-summary">{result.summary_cards.map((item) => <div key={item.label} className={`moni-tax-kpi ${item.tone || ''}`}><span>{item.label}</span><b>{item.value}</b></div>)}</div>
        <p className="moni-tax-basis">{result.basis}</p>
