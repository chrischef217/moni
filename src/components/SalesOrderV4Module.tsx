...
function orderMoney(row: Order, value: unknown) {
  return String(row.source_type || '').toUpperCase() === 'EXPORT' ? currencyMoney(value, String(row.currency || 'KRW').toUpperCase()) : money(value)
}
...
export default function SalesOrderV4Module({ mode = 'sales' }: { mode?: 'sales' | 'statements' }) {
  ...
  const visibleOrders = mode === 'statements' ? orders.filter((row) => row.status !== 'cancelled') : orders
  ...
  function openEdit(row: Order) { if (String(row.source_type || '').toUpperCase() === 'EXPORT' && row.source_reference) { window.location.href = `/sales-management/export/documents?edit=${encodeURIComponent(String(row.source_reference))}`; return } if (row.financial_locked) { setError('실제 입금이 등록된 판매건은 금액·품목을 수정할 수 없습니다. 수금·미수금 화면에서 입금을 먼저 확인해 주세요.'); return } if (row.items.some((item) => !item.sales_variant_id)) { setError('이 판매건은 이전 판매 구조로 등록되어 다중규격 화면에서 직접 수정할 수 없습니다. 기존 기록은 보존됩니다.'); return } setEditingId(row.id); setDraft({ sale_date: row.sale_date, client_id: row.client_id, status: row.status === 'draft' ? 'draft' : 'confirmed', vat_rate: String(row.vat_rate), note: row.note ?? '', items: row.items.map((item) => ({ sales_variant_id: item.sales_variant_id || '', quantity: String(item.quantity), unit_price: String(item.unit_price) })) }); setModal(true); setError('') }
  ...
  return <main className="min-h-screen bg-[#071426] px-4 py-6 text-slate-100 md:px-8"><div className="mx-auto max-w-[1600px] space-y-5">
    ...
    <section className="overflow-hidden rounded-3xl border border-slate-700 bg-slate-900/55"><div className="border-b border-slate-700 p-5"><h2 className="text-xl font-black">{mode === 'statements' ? '명세표 출력 목록' : '판매 내역'}</h2><p className="mt-1 text-sm text-slate-400">{mode === 'statements' ? '등록된 거래명세표는 바로 출력할 수 있습니다. 수출에서 생성된 명세표는 원본 수출서류에서 수정합니다.' : '실제 입금이 한 번이라도 등록되면 금액·품목 수정과 판매취소가 잠깁니다.'}</p></div><div className="overflow-x-auto"><table className="min-w-[1180px] w-full text-sm"><thead className="bg-slate-800 text-slate-400"><tr>{mode !== 'statements' && <th className="px-4 py-3 text-left">상태</th>}{['판매일', '명세표', '거래처', '판매 kg', '공급가액', '부가세', '합계', '수금상태', '입금예정일', '관리'].map((label) => <th key={label} className="px-4 py-3 text-left">{label}</th>)}</tr></thead><tbody>{visibleOrders.map((row) => { const totalKg = row.items.reduce((sum, item) => sum + Number(item.quantity_kg || 0), 0); const isExport = String(row.source_type || '').toUpperCase() === 'EXPORT' && Boolean(row.source_reference); return <tr key={row.id} className={`border-t border-slate-800 ${mode !== 'statements' && row.status === 'cancelled' ? 'opacity-40' : ''}`}>{mode !== 'statements' && <td className="px-4 py-4"><span className={row.status === 'confirmed' ? 'text-emerald-300' : row.status === 'draft' ? 'text-amber-300' : 'text-slate-500'}>{statusLabel(row.status)}</span>{row.financial_locked && <div className="mt-1 text-[10px] text-blue-300">입금 발생 · 금액 잠금</div>}</td>}<td className="px-4 py-4">{row.sale_date}</td><td className="px-4 py-4 font-bold text-blue-300">{row.statement_number}</td><td className="px-4 py-4 font-bold">{clientById.get(row.client_id)?.company_name || '-'}</td><td className="px-4 py-4">{qty(totalKg)}kg</td><td className="px-4 py-4">{orderMoney(row, row.supply_amount)}</td><td className="px-4 py-4">{orderMoney(row, row.vat_amount)}</td><td className="px-4 py-4 font-black text-emerald-200">{orderMoney(row, row.total_amount)}</td><td className="px-4 py-4"><button type="button" onClick={() => window.location.href = '/business-management?tab=sales-management&view=receivables'} className="rounded-lg border border-slate-700 px-2 py-1 text-xs hover:border-blue-500">{paymentLabel(row.payment_status)}{row.posted_receipt_amount > 0 ? ` · ${orderMoney(row, row.posted_receipt_amount)}` : ''}</button></td><td className="px-4 py-4">{row.due_date || <span className="text-slate-500">미설정</span>}</td><td className="px-4 py-4 whitespace-nowrap"><button type="button" onClick={() => printOrder(row)} className="mr-3 underline">출력</button>{isExport ? <button type="button" onClick={() => window.location.href = `/sales-management/export/documents?edit=${encodeURIComponent(String(row.source_reference))}`} className="underline text-blue-300">수출서류 수정</button> : <><button type="button" onClick={() => openEdit(row)} disabled={row.financial_locked} className="mr-3 underline disabled:opacity-30">수정</button><button type="button" onClick={() => { if (row.financial_locked) { setError('실제 입금이 등록된 판매건은 먼저 입금을 취소해야 삭제할 수 있습니다.'); return } setCancelOrderId(row.id); setCancelReason(mode === 'statements' ? '거래명세표 목록에서 삭제' : '') }} disabled={row.financial_locked} className="underline disabled:opacity-30">{mode === 'statements' ? '삭제' : '취소'}</button></>}</td></tr> })}{!visibleOrders.length && <tr><td colSpan={mode === 'statements' ? 10 : 11} className="px-5 py-14 text-center text-slate-500">조회 월에 등록된 판매가 없습니다.</td></tr>}</tbody></table></div></section>
  </div>
  ...
  {cancelOrderId && <div className="fixed inset-0 z-[1400] flex items-center justify-center bg-black/75 p-4"><div className="w-full max-w-lg rounded-3xl border border-red-500/30 bg-[#0f1b2d] p-6"><h2 className="text-xl font-black text-red-100">{mode === 'statements' ? '거래명세표 삭제' : '판매 취소'}</h2><p className="mt-2 text-sm leading-6 text-slate-400">{mode === 'statements' ? '목록에서는 삭제되며 판매 취소 이력은 보존됩니다. 실제 입금이 있는 건은 삭제할 수 없습니다.' : '취소 전 판매내용은 이력으로 보존됩니다. 실제 입금이 있는 판매는 이 화면에서 취소할 수 없습니다.'}</p><div className="mt-4"><Field label={mode === 'statements' ? '삭제 사유' : '취소 사유'}><input value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} placeholder="선택 입력" className={inputClass} /></Field></div><div className="mt-6 flex justify-end gap-3"><button type="button" onClick={() => setCancelOrderId('')} className={secondaryButton}>닫기</button><button type="button" disabled={saving} onClick={() => void cancelOrder()} className="rounded-xl bg-red-600 px-5 py-2.5 text-sm font-black text-white disabled:opacity-40">{mode === 'statements' ? '삭제' : '판매 취소 확정'}</button></div></div></div>}
  </main>
}
