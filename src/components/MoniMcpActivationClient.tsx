...
export default function MoniMcpActivationClient() {
  ...
  const [preflight, setPreflight] = useState<PreflightGate | null>(null)
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('moni-mcp-preflight-updated', refresh)
  }, [])
  ...
  const canOpen = Boolean(preflight?.ready)
  ...
  return (
    <section className="mb-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-7">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm leading-6 text-slate-500">
            실제 ChatGPT 연결시험에만 최대 30분 동안 임시로 엽니다. 최근 30분 내 Preflight PASS와 현재 도구 카탈로그 해시 일치가 필수입니다.
          </p>
        </div>
      ...
      <div className={`mt-4 rounded-xl border px-4 py-3 text-sm ${canOpen ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-amber-200 bg-amber-50 text-amber-900'}`}>
        <p className="font-black">Preflight Gate: {canOpen ? 'PASS — 수용검사 창 개방 가능' : '잠김'}</p>
        <p className="mt-1 text-xs leading-5">{preflight?.reason || '사전점검 상태를 확인 중입니다.'}</p>
        {preflight?.expires_at && <p className="mt-1 text-xs">유효 종료: {formatDate(preflight.expires_at)}</p>}
      </div>
      ...
      {!permanent && !acceptance && (
        <div className="mt-5 grid gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 md:grid-cols-[1fr_140px_auto]">
          ...
          <button
            type="button"
            disabled={working || reason.trim().length < 3 || !canOpen}
            onClick={() => {
            ...
            className="self-end rounded-xl bg-amber-600 px-4 py-3 text-sm font-black text-white hover:bg-amber-700 disabled:cursor-not-allowed disabled:opacity-40"
          >
            수용검사 창 열기
          </button>
        </div>
