...
function resultCard(value: unknown): ResultCard {
  if (!value || typeof value !== 'object') return { title: '처리 완료', lines: [] }
  ...
  if (nested.result && typeof nested.result === 'object' && (Array.isArray(nested.result.lines) || Array.isArray(nested.result.links))) return nested.result as ResultCard
  const preferred = ['quote_number','invoice_number','category_label','record_id','saved','status','verified']
  const lines = preferred.filter((key) => nested[key] !== undefined && nested[key] !== null).map((key) => `${key}: ${typeof nested[key] === 'object' ? JSON.stringify(nested[key]) : String(nested[key])}`)
...
export default function MoniMobileCapabilityV4() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      // Supplemental business card must never block core chat.
    }
  }, [host])
  ...
  const completed = resultCard(card.result)
  return createPortal(<>
