...
function syntheticSuccess(text: string, threadId: string, extra: Record<string, unknown> = {}) {
  return new Response(JSON.stringify({ ok: true, text, thread_id: threadId, recovered_background_turn: true, ...extra }), { status: 200, headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' } })
}
function friendlyRecoveryFailure(threadId: string) {
  return new Response(JSON.stringify({ ok: false, thread_id: threadId, code: 'MONI_BACKGROUND_RECOVERY_PENDING', error: '화면을 다시 열었습니다. 서버 작업 결과를 아직 확인 중입니다. 잠시 후 이 대화에서 자동으로 이어집니다.' }), { status: 503, headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' } })
}
function completedAssistant(messages: StoredMessage[], expectedUser: string) {
  let userIndex = -1
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const row = messages[index]
    if (row?.role === 'user' && normalize(row.content) === expectedUser) { userIndex = index; break }
  }
...
async function ensureThreadId(originalFetch: typeof window.fetch, body: AgentBody) {
  ...
  const response = await originalFetch('/api/moni/mobile-thread', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ thread_id: existing || undefined, page: body.page || {} }), cache: 'no-store' })
...
async function matchingActionCard(originalFetch: typeof window.fetch, threadId: string, expectedUser: string, intent: MobileBusinessIntent) {
  try {
    const endpoint = intent.domain === 'raw_material_inbound' ? '/api/moni/mobile-actions' : '/api/moni/mobile-business-actions'
    const [cardResponse, messages] = await Promise.all([
      originalFetch(`${endpoint}?thread_id=${encodeURIComponent(threadId)}&_=${Date.now()}`, { cache: 'no-store' }),
      loadThreadMessages(originalFetch, threadId),
    ])
    const cardPayload = await cardResponse.json().catch(() => ({})) as { ok?: boolean; card?: { stage?: string; operation?: string; domain?: string } | null }
...
async function recoverFinishedTurn(originalFetch: typeof window.fetch, threadId: string, expectedUser: string) {
  ...
  while (Date.now() < deadline) {
    ...
    } catch { /* retry after foreground resume */ }
...
export default function MoniMobileContinuityGuard() {
  useLayoutEffect(() => {
    ...
    const wrappedFetch: typeof window.fetch = async (input, init) => {
      ...
      try { threadId = await ensureThreadId(originalFetch, parsed) } catch (error) {
        return new Response(JSON.stringify({ ok: false, error: error instanceof Error ? error.message : 'MONI 대화방을 준비하지 못했습니다.' }), { status: 503, headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' } })
      }
      ...
      if (coreIntent && attachmentCount > 0) {
        ...
        const first = await Promise.race([actual, cardRace.then((response) => response ? ({ type: 'card' as const, response }) : ({ type: 'none' as const }))])
        if (first.type === 'card') { void actual.then(() => undefined); return first.response }
