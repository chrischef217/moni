...
function ExporterBlock({ profile }: { profile: Record<string, any> }) {
  const company = profile.company_name_en || profile.company_name_ko || 'DOOBAE'
  const address = profile.address_en || profile.address_ko || ''
...
function Signature({ profile }: { profile: Record<string, any> }) {
  return <div className="signature"><div className="signature-line">Authorized Signature</div>{profile.signature_data_url ? <img src={profile.signature_data_url} alt="Authorized signature" /> : <div className="signature-placeholder" /> }<b>{profile.representative_name_en || profile.representative_name_ko || ''}</b><span>{profile.company_name_en || profile.company_name_ko || ''}</span></div>
}
...
export default function ExportDocumentPrintView({ id, type, autoPrint }: { id: string; type: PrintType; autoPrint: boolean }) {
  ...
  return <>
    ...
    {showInvoice && <article className="paper invoice-paper">
      ...
      <table className="doc-table"><thead><tr><th>No.</th><th>Description of Goods</th><th>Qty</th><th>EA / CTN</th><th>Unit Price / CTN</th><th>Amount</th></tr></thead><tbody>{document.export_document_items.map((item, index) => <tr key={item.id}><td>{index + 1}</td><td className="left"><b>{item.product_name_en}</b><small>{item.product_name_ko}</small></td><td>{item.cartons} CTN</td><td>{item.units_per_carton}</td><td>{money(Number(item.unit_price), item.currency)}</td><td>{money(Number(item.cartons) * Number(item.unit_price), item.currency)}</td></tr>)}</tbody></table>
    ...
    {showPacking && <article className={`paper packing-paper ${showInvoice ? 'page-break' : ''}`}>
      ...
      <table className="doc-table packing"><thead><tr><th>No.</th><th>Description</th><th>CTN</th><th>EA / CTN</th><th>Total EA</th><th>Net Wt.</th><th>Gross Wt.</th><th>CBM</th></tr></thead><tbody>{document.export_document_items.map((item, index) => { const cartons = Number(item.cartons); return <tr key={item.id}><td>{index + 1}</td><td className="left"><b>{item.product_name_en}</b><small>{item.product_name_ko}</small></td><td>{cartons}</td><td>{item.units_per_carton}</td><td>{cartons * Number(item.units_per_carton)}</td><td>{number(cartons * Number(item.net_weight_per_carton_kg))} kg</td><td>{number(cartons * Number(item.gross_weight_per_carton_kg))} kg</td><td>{number(cartons * Number(item.cbm_per_carton), 6)}</td></tr> })}</tbody></table>
    ...
    <style jsx global>{`
      ...
      .paper { box-sizing: border-box; width: 210mm; min-height: 297mm; margin: 18px auto; padding: 12mm 13mm; background: white; box-shadow: 0 10px 30px rgba(20, 42, 57, .14); font-size: 10.5px; line-height: 1.35; }
      ...
      .lc { margin-top: 3mm; }
      .doc-table { width: 100%; margin-top: 5mm; border-collapse: collapse; table-layout: fixed; }
      .doc-table th, .doc-table td { border: 1px solid #999; padding: 2.1mm 1.5mm; text-align: center; vertical-align: middle; }
      .doc-table th { background: #f1f3f4; font-size: 9px; }
      .doc-table th:nth-child(1) { width: 7%; }
      .doc-table th:nth-child(2) { width: 34%; }
      ...
      .packing th:nth-child(2) { width: 27%; }
