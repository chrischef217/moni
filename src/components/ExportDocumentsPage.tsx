...
export default function ExportDocumentsPage() {
  ...
  async function save(printAfter = false) {
    ...
    setSaving(true)
    try {
      if (form.id && form.status === 'CANCELLED') {
        const reopenResponse = await fetch('/api/moni/export-documents-reopen', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ id: form.id }),
        })
        const reopenPayload = await reopenResponse.json()
        if (!reopenResponse.ok || !reopenPayload.ok) throw new Error(reopenPayload.error || '취소된 수출서류를 수정 상태로 전환하지 못했습니다.')
      }

      const desiredStatus = printAfter ? 'GENERATED' : (form.status === 'GENERATED' || form.status === 'CANCELLED' ? 'GENERATED' : 'DRAFT')
  ...
  return <main className="min-h-screen bg-transparent px-4 py-5 text-[#17384d] md:px-6">
      ...
      <section className="overflow-hidden rounded-[26px] border border-[#cfe1eb] bg-white/95 shadow-[0_12px_34px_rgba(43,84,109,0.07)]">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1120px] border-collapse text-sm">
            ...
            <tbody>{documents.map((document) => { const total = totals(document); return <tr key={document.id} className="border-t border-[#e7eff4] bg-white hover:bg-[#f9fcfd]"><td className="px-5 py-4 whitespace-nowrap">{document.document_date}</td><td className="px-4 py-4 whitespace-nowrap font-black text-[#31546a]">{document.invoice_no}</td><td className="px-4 py-4 whitespace-nowrap font-bold text-[#5c7685]">{document.packing_list_no}</td><td className="px-4 py-4 font-black">{document.consignee_snapshot?.company_name || '-'}</td><td className="px-4 py-4">{document.consignee_snapshot?.country || '-'}</td><td className="px-4 py-4 text-center font-black">{total.cartons}</td><td className="px-4 py-4 whitespace-nowrap font-black text-[#176f99]">{total.amountText || '-'}</td><td className="px-4 py-4 text-center"><span className={`inline-flex rounded-lg px-2.5 py-1.5 text-xs font-black ${statusClass(document.status)}`}>{statusLabel(document.status)}</span></td><td className="px-5 py-4"><div className="flex flex-wrap justify-center gap-1.5">{document.status !== 'SHIPPED' && <button type="button" onClick={() => openEdit(document)} className="rounded-lg border border-[#bfd5e1] bg-white px-2.5 py-2 text-xs font-black text-[#315d75]">수정</button>}<button type="button" onClick={() => window.open(`/sales-management/export/documents/${document.id}/print?type=invoice`, '_blank')} className="rounded-lg border border-[#c8d9e3] bg-white px-2.5 py-2 text-xs font-bold">Invoice</button><button type="button" onClick={() => window.open(`/sales-management/export/documents/${document.id}/print?type=packing`, '_blank')} className="rounded-lg border border-[#c8d9e3] bg-white px-2.5 py-2 text-xs font-bold">Packing</button><button type="button" onClick={() => window.open(`/sales-management/export/documents/${document.id}/print?type=both&auto=1`, '_blank')} className="rounded-lg bg-[#315d75] px-2.5 py-2 text-xs font-black text-white">PDF/인쇄</button>{document.status !== 'SHIPPED' && document.status !== 'CANCELLED' && <button type="button" onClick={() => void changeStatus(document, 'SHIP')} className="rounded-lg bg-[#16b981] px-2.5 py-2 text-xs font-black text-white">출고확정</button>}{document.status === 'SHIPPED' && <button type="button" onClick={() => void changeStatus(document, 'CANCEL')} className="rounded-lg border border-[#efc0c4] bg-[#fff7f7] px-2.5 py-2 text-xs font-black text-[#b24c55]">출고취소</button>}{document.status !== 'SHIPPED' && <button type="button" onClick={() => void remove(document)} className="rounded-lg border border-[#efc0c4] bg-white px-2.5 py-2 text-xs font-bold text-[#b24c55]">삭제</button>}</div></td></tr> })}{!documents.length && <tr><td colSpan={9} className="px-6 py-16 text-center text-[#8296a3]">작성된 수출서류가 없습니다.</td></tr>}</tbody>
          </table>
        </div>
      </section>
    </div>
    ...
    {modalOpen && <div className="fixed inset-0 z-[1300] flex items-center justify-center bg-[rgba(12,31,44,0.34)] p-4 backdrop-blur-[3px]" onMouseDown={(event) => { if (event.currentTarget === event.target && !saving) setModalOpen(false) }}>
      <div className="flex max-h-[94vh] w-full max-w-[1180px] flex-col overflow-hidden rounded-[26px] border border-[#cfe1eb] bg-white shadow-[0_28px_80px_rgba(22,52,72,0.24)]">
        <div className="flex items-start justify-between gap-4 border-b border-[#dce9f0] px-6 py-5"><div><p className="text-xs font-black uppercase tracking-[0.14em] text-[#2b9b76]">EXPORT DOCUMENT</p><h2 className="mt-1 text-2xl font-black">{form.id ? '수출서류 수정' : '수출서류 작성'}</h2>{form.status === 'CANCELLED' && <p className="mt-1 text-sm font-semibold text-[#7a8e9a]">취소된 서류를 저장하면 다시 서류생성 상태로 전환됩니다.</p>}</div><button type="button" onClick={() => setModalOpen(false)} disabled={saving} className="rounded-xl border border-[#d0e0e8] bg-white px-4 py-2.5 text-sm font-bold text-[#587283]">닫기</button></div>
