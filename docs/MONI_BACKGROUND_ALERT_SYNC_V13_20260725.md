...
개발 중 Preview가 생성되지 않은 시점이 있었으나, 이후 GitHub/Vercel 상태에서 원인이 `upgradeToPro=build-rate-limit`로 확인됐다. 즉 Cron 주기 제한 문제가 아니라 단시간에 많은 Preview 빌드를 생성해 Vercel 빌드 생성 제한에 걸린 것이었다. 따라서 시간당 Cron 설계를 유지한다.

관리자 웹이 열려 있는 동안에는 V10 `GlobalAlertSyncController`가 15분마다 동기화하므로 실제 구조는:

- 웹 사용 중: 최대 15분 간격
- 웹 미사용 중: 최대 1시간 간격 서버 점검
