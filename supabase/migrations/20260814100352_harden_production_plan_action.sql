-- Harden production-plan execution to the same transaction boundary as
-- production-record actions. Existing operational rows are not changed.
...
-- The legacy two-argument overload cannot bind execution to the authenticated
-- MCP actor. Keep it unavailable to every API role during rollout.
