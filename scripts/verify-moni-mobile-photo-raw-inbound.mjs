...
const checks = [
  ...
  [touch.includes("querySelectorAll<HTMLElement>('button, textarea, input, a')"), 'photo recovery must release stale locks from interactive children'],
  [touch.includes("querySelector('.moni-new-chat-button')"), 'photo recovery must verify the new-conversation control is reachable'],
  ...
  [touch.includes('window.setInterval(() =>'), 'idle mobile interaction must be revalidated periodically after photo answers'],
  [touch.includes("candidate !== document.body && candidate !== root"), 'internal non-dialog full-screen blockers must be releasable without disabling the MONI root'],
