...
const checks = [
  ...
  [touch.includes("computed.pointerEvents === 'none'"), 'photo recovery must release computed pointer-events locks, not only inline locks'],
  [touch.includes('normalizeIdleControls'), 'idle mobile controls must recover stale disabled state'],
  [touch.includes("root.querySelector<HTMLButtonElement>('.moni-new-chat-button')"), 'photo recovery must verify the new-conversation control is reachable'],
  [touch.includes("document.addEventListener('pointerdown', onPointerDown, true)"), 'idle mobile taps must trigger interaction self-recovery'],
  [touch.includes("document.addEventListener('touchstart', onPointerDown, true)"), 'Android touchstart must trigger interaction self-recovery'],
  [touch.includes('window.setInterval(repairNow, RELEASE_INTERVAL_MS)'), 'idle mobile interaction must be revalidated periodically after completed answers'],
  [touch.includes('candidate.dataset.moniReleasedTouchBlocker'), 'stale non-dialog layers above core controls must be releasable'],
  [touch.includes("candidate.style.setProperty('pointer-events', 'none', 'important')"), 'released blockers must not immediately reclaim touch through CSS specificity'],
  [!touch.includes('if (!top || root.contains(top)) return'), 'photo recovery must not ignore stale blockers mounted inside the MONI root'],
