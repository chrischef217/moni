...
for (const [name, source] of blockedRoutes) {
  ...
  if (source.includes('OPENAI_API_KEY') || source.includes('runMoniSdkAgent') || source.includes("from '@openai/agents'")) failures.push(`${name} must not call a server-side AI model`)
}
