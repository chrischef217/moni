...
const v6 = readFileSync('src/app/api/moni/mobile-capability-v6/route.ts', 'utf8')
...
requireText(middleware, "pathname === '/api/moni/mobile-capability-v4'", 'mobile V4 requests must use the hardened compatibility bridge')
requireText(middleware, "url.pathname='/api/moni/mobile-capability-v6'", 'mobile V6 rewrite target is missing')
...
requireText(v6, "body.domain === 'hr_required_document'", 'V6 must intercept HR document reads before legacy V4')
requireText(v6, ".in('id', attachmentIds)", 'HR document reads must resolve attachments across prior threads')
requireText(v6, "intent.domain !== 'hr_required_document' || intent.operation !== 'READ'", 'HR read bridge must revalidate current user intent')
...
if (failures.length) {
  console.error('MONI mobile hardened source verification failed:')
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}

console.log('MONI mobile hardened source verification passed.')
